      ...
      <div className="bg-[#151521] border border-white/[0.08] rounded-2xl shadow overflow-hidden">
        ...
        <div className="divide-y divide-white/[0.08]">
          {isLoading ? (
            ...
            <div key={u.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-[#151521] transition-colors">
              <span className="text-lg font-black text-[#6B7280] w-6 text-center">{i + 1}</span>
              <img src={u.avatar} alt={u.name} className="w-10 h-10 rounded-full" />
              <div className="flex-1">
                <p className="text-sm font-semibold text-white">{u.name}</p>
                <p className="text-xs text-[#9CA3AF]">{u.hostel}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-bold text-[#FF8C00]">{u.listings} listings</p>
                <p className="text-xs text-[#9CA3AF]">{u.sold} sold</p>
              </div>
            </div>
          ))}
        </div>

...
const ChatRow = memo(function ChatRow({ chat, currentUserId, isOnline }: { chat: Chat; currentUserId: string; isOnline: boolean }) {
  ...
  return (
    <Link href={`/chats/${chat.id}`}
      className="flex items-center gap-3 px-4 py-3.5 hover:bg-[#151521] transition-colors cursor-pointer border-b border-white/[0.08] last:border-0">
      <div className="relative flex-shrink-0">
        <img src={otherAvatar} alt={otherName} className="w-11 h-11 rounded-full bg-[#1E1E2E]" />
      ...
      <div className="flex-1 min-w-0">
        ...
        {chat.productTitle && (
          <p className="text-[11px] text-[#FF8C00] font-medium truncate mt-0.5">{chat.productTitle}</p>
        )}
      ...
      {isUnread && chat.unreadCount > 0 && (
        <div className="w-5 h-5 rounded-full bg-[#FF6B00] text-white text-[10px] font-bold flex items-center justify-center flex-shrink-0">
  ...
  return (
    <div className="min-h-screen bg-[#0D0D1A]">
      <div className="max-w-2xl mx-auto">
        {/* Header */}
        <div className="sticky top-14 z-10 bg-[#0D0D1A]/95 backdrop-blur-xl border-b border-white/[0.08] px-4 py-4">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-xl bg-[#151521] flex items-center justify-center">
            ...
            {chats.filter(c => c.unreadCount > 0 && c.lastMessageSenderId !== currentUserId).length > 0 && (
              <span className="ml-1 bg-[#FF6B00] text-white text-xs font-bold px-2 py-0.5 rounded-full">
        ...
        {/* Chat list */}
        <div className="bg-[#151521] border border-white/[0.08] shadow-[0_2px_8px_rgba(0,0,0,0.30)] mx-4 my-4 rounded-2xl overflow-hidden">
          {isLoading ? (
            <div className="flex flex-col gap-3 p-4">
              {[1,2,3].map(i => (
                <div key={i} className="flex items-center gap-3 animate-pulse">
                  <div className="w-11 h-11 rounded-full bg-[#1E1E2E]" />
